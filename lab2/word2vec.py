...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    #m=softmax(np.dot(centerWordVec,outsideVectors.T))
    #loss = -np.log(m[])
    #loss = - np.log(np.exp(np.dot(centerWordVec,outsideVectors[])) / )
...
def negSamplingLossAndGradient(
    ...
    Arguments/Return Specifications: same as naiveSsoftmaxLossAndGradient
    """
    ...
    print(centerWordVec.shape[0])
    ### YOUR CODE HERE (~10 Lines)
    
    n = sorted(set(negSampleWordIndices))
    length = max(n) + 1
    arr1=[]
    for i in range(length):
       arr1.append(i)
    arr2 = [0 for i in range(length)]
    dic = zip(arr1,arr2)
    for i in negSampleWordIndices:
       dic[i]+=1
    m = np.dot(centerWosrdVec,outsideVectors)
    for i in negSampleWordIndices:
        sumresult += log(sigmoid(-m[i]))
    loss = -log(sigmoid(outsideVectors[outsideWordIdx]))-sumresult
    sum2 = np.zeros((1,3))
    for i in negSampleWordIndices:
        sum2 = np.sum(sum2,(1-sigmoid(-np.dot(centerWordVec,outsideVectors[i]))) * outsideVectors[i])
    gradCenterVec = np.sum(sum2,-(1-sigmoid(np.dot(centerWordVec,outsideVectors[outsideWordIdx]))) * outsideVectors[outsideWordIdx])
